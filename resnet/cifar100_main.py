...
def get_filenames(is_training, data_dir):
  ...
  data_dir = os.path.join(data_dir, 'cifar-100-batches-bin')
...
class Cifar100Model(resnet.Model):
  ...
  def __init__(self, resnet_size, data_format=None, squeeze_excitation_fn=None,
...
def cifar100_model_fn(features, labels, mode, params):
  ...
  # for the CIFAR-100 dataset, perhaps because the regularization prevents
  # overfitting on the small data set. We therefore include all vars when
  # regularizing and computing loss during training.
  def loss_filter_fn(name):
    return True

  if params['squeeze_excitation_fn'] == '':
    squeeze_excitation_fn = None
  elif params['squeeze_excitation_fn'] == 'mean':
    squeeze_excitation_fn = resnet.avg_squeeze_and_excitation
  elif params['squeeze_excitation_fn'] == 'max':
    squeeze_excitation_fn = resnet.max_squeeze_and_excitation
  elif params['squeeze_excitation_fn'] == 'mean_var':
    squeeze_excitation_fn = resnet.mean_var_squeeze_and_excitation
  else:
    raise ValueError("Unknwon squeeze_excitation_fn '{}'."
                     "".format(params['squeeze_excitation_fn']))
...
if __name__ == '__main__':
  ...
  parser.set_defaults(data_dir='/tmp/cifar100_data',
                      model_dir='/tmp/cifar100_model',
                      resnet_size=32,
                      train_epochs=250,
                      epochs_per_eval=10,
                      batch_size=128)
